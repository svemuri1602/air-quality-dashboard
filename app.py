...
def plot_data(df, column, time_col, prefix):
    if df.empty:
        st.warning("No data available for the selected filters.", key=f"{prefix}_warning")
        return

    st.subheader("Summary Statistics", key=f"{prefix}_stats_header")
    st.write(df[[column]].describe(), key=f"{prefix}_stats")

    if column.lower() == 'pm2.5' and df[column].max() > 100:
        st.error("⚠️ Alert: PM2.5 has exceeded 100 at some points in the selected data.", key=f"{prefix}_alert")

    max_points = 1000
    if len(df) > max_points:
        df = df.sample(n=max_points).sort_values(time_col)

    st.line_chart(df.set_index(time_col)[column], key=f"{prefix}_linechart")
    st.bar_chart(df.set_index(time_col)[column], key=f"{prefix}_barchart")

    st.write("Correlation Heatmap", key=f"{prefix}_heatmap_header")
    corr = df.select_dtypes(include='number').corr()
    fig, ax = plt.subplots(num=f"{prefix}_heatmap_fig")
    sns.heatmap(corr, annot=True, ax=ax)
    st.pyplot(fig, key=f"{prefix}_heatmap")
    plt.close(fig)
